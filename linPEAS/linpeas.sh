...
if [ "`echo $CHECKS | grep IntFiles`" ]; then
  ...
  if ! [ "$SUPERFAST" ] && [ "$TIMEOUT" ]; then
    ...
    intpwdfiles=`timeout 120 grep -RiIE "(pwd|passwd|password|PASSWORD).*[=:].+|define ?\('(\w*passw|\w*user|\w*datab)" $HOMESEARCH /var/www $backup_folders_row /tmp /etc /root /mnt /Users /private 2>/dev/null`
